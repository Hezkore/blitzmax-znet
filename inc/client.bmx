...
Type TZClient Extends TSocketStream
	...
	'PACKET SENDING
	rem
	Method StartPacket(packetID:Byte, toClient:Short = 0)
		packetOffset = 0
		packetBank = CreateBank(zNetHeader)
		
		'Insert packetID
		packetBank.PokeByte(packetOffset, packetID)
		packetOffset:+1
		
		'Insert toClient
		packetBank.PokeShort(packetOffset, toClient)
		packetOffset:+2
		
		'We skip TWO at the end to leave space Length:Short (added at SendPacket)
		packetOffset:+2
	End Method
	
	Method ConnectPacket()
		packetOffset = 0
		packetBank = CreateBank(zNetHeader)
		
		'Insert packetID
		packetBank.PokeByte(packetOffset, zNet_ID_Join)
		packetOffset:+1
		
		'Insert toClient
		packetBank.PokeShort(packetOffset, 0)
		packetOffset:+2
	...
	Method AddByte(Value:Byte)
		ResizeBank(packetBank, packetBank.Size() + 1)
		packetBank.PokeByte(packetOffset, Value)
		packetOffset:+1
	End Method
	
	Method AddShort(Value:Short)
		ResizeBank(packetBank, packetBank.Size() + 2)
		packetBank.PokeShort(packetOffset, Value)
		packetOffset:+2
	End Method
	
	Method AddInt(Value:Int)
		ResizeBank(packetBank, packetBank.Size() + 4)
		packetBank.PokeInt(packetOffset, Value)
		packetOffset:+4
	End Method
	
	Method AddLong(Value:Long)
		ResizeBank(packetBank, packetBank.Size() + 8)
		packetBank.PokeLong(packetOffset, Value)
		packetOffset:+8
	End Method
	
	Method AddFloat(Value:Float)
		ResizeBank(packetBank, packetBank.Size() + 4)
		packetBank.PokeFloat(packetOffset, Value)
		packetOffset:+4
	End Method
	
	Method AddDouble(Value:Double)
		ResizeBank(packetBank, packetBank.Size() + 8)
		packetBank.PokeDouble(packetOffset, Value)
		packetOffset:+8
	End Method
	
	Method AddString(Text:String)
		'Add length of string first
		ResizeBank(packetBank, packetBank.Size() + 2)
		packetBank.PokeShort(packetOffset, Len(Text))
		packetOffset:+2
		
		'Add text as bytes
		ResizeBank(packetBank, packetBank.Size() + Len(Text))
		For Local i:Int = 1 To Len(Text)
			packetBank.PokeByte(packetOffset, Asc(Mid(Text, i, 1)))
			packetOffset:+1
		Next
	End Method
	...
	Method SendPacket()
		...
		packetBank.PokeShort(zNetHeader - 2, BankSize(packetBank))
		
		'Add the packet to the send list (Sent on Update)
		If Not _socket Or packetBank.PeekByte(0) = zNet_ID_Join Then
			ListAddFirst(packetList, packetBank)
		Else
			WriteBank(packetBank, Self._stream, 0, BankSize(packetBank))
		EndIf
		sent:+BankSize(packetBank)
	End Method
	
	Method getConnection:ztClientConnection(sessionID:Short)
		If connections[sessionID] Then Return(connections[sessionID])
	End Method
	Endrem
EndType

rem
Type ztClientConnection Extends TStream
	Field _socket:TSocket
	
	Field sessionID:Short
	Field state:Byte
	
	'Holds any random information you want!
	Field clientInfo:TMap
	
	Method New()
		clientInfo = CreateMap()
	End Method
EndType
Endrem